...
- **WHEN** an authenticated user with role 'admin' queries all rows from `public.profiles`
- **THEN** the database returns all customer profile records successfully

#### Scenario: Non-Admin Fetching Other Profiles
- **WHEN** an authenticated user without role 'admin' attempts to query other users' profiles from `public.profiles`
